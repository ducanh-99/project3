...
class BaseService:
    ...
    def filter_with_list_params(self, query, request_params):
        ...
        for field, operator, value in list(zip(
                request_params.field_values.split(
                    ';') if request_params.field_values else [],
                request_params.operators.split(
                    ';') if request_params.operators else [],
                request_params.values.split(
                    ';') if request_params.values else []
        )):
            if field and operator and value:
                _query = self.add_filter(
                    query=query, field=field, operator=operator, value=value)
